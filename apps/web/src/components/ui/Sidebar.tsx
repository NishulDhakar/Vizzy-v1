...
const ConvItem: React.FC<ConvItemProps> = ({ conv, activeId, onSelect, onDelete, onClose }) => {
...
const Group: React.FC<GroupProps> = ({ label, items, activeId, onSelect, onDelete, onClose }) => {
  ...
  return (
    <div className="mb-4">
      ...
      {items.map(c => (
        <ConvItem key={c.id} conv={c} activeId={activeId} onSelect={onSelect} onDelete={onDelete} onClose={onClose} />
      ))}
    </div>
  );
};
    ...
    <>
      ...
      `}>
        ...
        <div className="flex items-center justify-between px-4 pt-5 pb-4 border-b border-[var(--vz-line)]">
          ...
          <button
            onClick={onClose}
            className="md:hidden w-7 h-7 flex items-center justify-center rounded-[6px] text-[var(--vz-fg-2)] hover:bg-[var(--vz-bg-3)] hover:text-[var(--vz-fg-0)] transition-colors border-none bg-transparent cursor-pointer"
          >
        ...
        <div className="px-3 pt-3 pb-2">
          ...
          >
            <IconPlus size={14} />
            <span>New chat</span>
            <span className="ml-auto font-mono text-[10px] text-[var(--vz-fg-3)] border border-[var(--vz-line-strong)] rounded px-1 hidden sm:inline">
              ⌘N
            </span>
          </button>
        </div>
        ...
        <div className="px-3 pb-2">
          <div className="flex items-center gap-2 px-3 py-2 rounded-[8px] bg-[var(--vz-bg-2)] border border-[var(--vz-line)]">
            ...
            <input
              ...
              className="flex-1 bg-transparent border-none outline-none focus:outline-none focus:ring-0 focus:border-none text-[12px] text-[var(--vz-fg-1)] placeholder:text-[var(--vz-fg-3)] font-[var(--vz-font-body)]"
            />
          </div>
        </div>
        ...
        <div className="flex-1 overflow-auto px-1.5 pt-1 pb-2">
          {conversations.length === 0 ? (
            ...
            <>
              <Group label="Today"       items={today}     activeId={activeId} onSelect={onSelect} onDelete={onDelete} onClose={onClose} />
              <Group label="Yesterday"   items={yesterday} activeId={activeId} onSelect={onSelect} onDelete={onDelete} onClose={onClose} />
              <Group label="Last 7 days" items={last7}     activeId={activeId} onSelect={onSelect} onDelete={onDelete} onClose={onClose} />
              <Group label="Earlier"     items={earlier}   activeId={activeId} onSelect={onSelect} onDelete={onDelete} onClose={onClose} />
            </>
          )}
        </div>
